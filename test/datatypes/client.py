...
class TestClient(TestDataType):
    ...
    def test_hashing(self):
        # test equal hash
        c_equal = InternalClient(name='c', commodity='c_com_2', demands=self.SERIES_2, variance_fn=self.VARIANCE_2)
        self.assertEqual(self.CLIENT, c_equal, msg="Nodes with the same name should be considered equal")
        # test different hash
        c_diff = InternalClient(name='cd', commodity='c_com', demands=self.SERIES_1, variance_fn=self.VARIANCE_1)
    ...
    def test_properties(self):
        ...
        self.assertEqual(self.CLIENT.commodity_in, 'c_com', msg="Wrong client inputs stored")
